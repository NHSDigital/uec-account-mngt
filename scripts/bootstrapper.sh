...
if [ -z "$ACCOUNT_ENVIRONMENT" ] ; then
  echo Set ACCOUNT_ENVIRONMENT to identify if account is for dev, test, security, preprod or prod
  EXPORTS_SET=1
else
  if [[ ! $ACCOUNT_ENVIRONMENT =~ ^(dev|test|preprod|prod|security) ]]; then
      echo ACCOUNT_ENVIRONMENT should be dev test preprod security or prod
      EXPORTS_SET=1
  fi
fi
...
export TERRAFORM_BUCKET_NAME="nhse-uec-$ACCOUNT_PROJECT-$ACCOUNT_ENVIRONMENT-terraform-state"    # A globally unique name for your terraform remote state bucket
export TF_VAR_account_alias="nhse-uec-$ACCOUNT_PROJECT-$ACCOUNT_ENVIRONMENT"
...
/bin/bash ./scripts/infra-deploy.sh plan terraform_management $ACCOUNT_ENVIRONMENT false
...
# TODO change plan to apply
/bin/bash ./scripts/infra-deploy.sh plan github-runner "$ACCOUNT_ENVIRONMENT"
